...
//                        BufferedReader r = new BufferedReader(new InputStreamReader(socket.getInputStream()));// 客户端输入
//                        PrintWriter w = new PrintWriter(socket.getOutputStream());// 客户端
//                        // 接收客户端的基本用户信息
...
//                    }
                    ...
                    clients.add(client);
                    listModel.addElement(client.getUser().getName());// 更新在线列表////////////////、、、、、、////////////都谁在线了
    ...
    class ClientThread extends Thread {
        ...
        public ClientThread(Socket socket) {
            try {
                this.socket = socket;
                reader = new BufferedReader(new InputStreamReader(socket.getInputStream()));// 从客户端输入到服务端的
        ...
        public void dispatcherMessage(String message) {
            ...
            if (owner.equals("ALL")) {// 群发
                ...
                for (int i = clients.size() - 1; i >= 0; i--) {
                    if (clients.get(i).user.getName().equals(owner)) {////////////////////////// 得到要发给谁的id与owner对比，同就发
                        
                        clients.get(i).getWriter().println(message + "(单人发送)");
                        clients.get(i).getWriter().flush();
                        break;
                    }
                }
